// $Id$
/*
 * MIOS Terminal interface to lwIP parameters
 *
 * ==========================================================================
 *
...
{
  void (*out)(char *format, ...) = _output_function;
  const u8 *mac = LWIP_TASK_MAC_AddressGet();
...
static s32 UIP_TERMINAL_PrintIPs(void *_output_function)
{
  ...
  u32 ipaddr = LWIP_TASK_IP_EffectiveAddressGet();
  out("IP address: %d.%d.%d.%d",
      ipaddr >> 24, (ipaddr >> 16) & 0xff, (ipaddr >> 8) & 0xff, ipaddr & 0xff);

  u32 netmask = LWIP_TASK_EffectiveNetmaskGet();
  out("Netmask: %d.%d.%d.%d",
      netmask >> 24, (netmask >> 16) & 0xff, (netmask >> 8) & 0xff, netmask & 0xff);

  u32 draddr = LWIP_TASK_EffectiveGatewayGet();
  out("Default Router (Gateway): %d.%d.%d.%d",
      draddr >> 24, (draddr >> 16) & 0xff, (draddr >> 8) & 0xff, draddr & 0xff);
